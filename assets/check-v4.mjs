...
for (const [file, hash] of Object.entries(manifest.sources ?? {}))
  assert.equal(createHash('sha256').update(await readFile(new URL(file, source))).digest('hex'), hash, `${file} source hash`);
const loaded = new Map();
for (const name of names) {
  const contract = manifest.contract[name];
  assert.ok(manifest.assets?.[contract.file]?.props?.[name], `${name}: v4 model has not been built/exported`);
  if (!loaded.has(contract.file)) {
    const bytes = await readFile(new URL(`../public/models/${contract.file}.glb`, source));
    ...
    assert.equal(validation.issues.numWarnings, 0, JSON.stringify(validation.issues));
    assert.equal(createHash('sha256').update(bytes).digest('hex'), manifest.assets[contract.file].sha256, `${contract.file} hash`);
  ...
  size.forEach((value, i) => assert.ok(value >= contract.min[i] && value <= contract.max[i], `${name} metre dimension ${i}: ${value}`));
  const semantic = new Map(); let triangles = 0;
  root.traverse(child => {
    near(child.scale.toArray(), [1, 1, 1], `${name}/${child.name} applied scale`);
    ...
    assert.ok(child.userData.partName || (child.isMesh && child.parent.userData.partName), `${name}/${child.name} semantic node or material-primitive parent`);
    if (child.userData.partName) {
      assert.ok(!semantic.has(child.userData.partName), `${name} unique semantic ${child.userData.partName}`);
      semantic.set(child.userData.partName, child);
    }
    if (child.isMesh) triangles += (child.geometry.index?.count ?? child.geometry.attributes.position.count) / 3;
  });
  assert.equal(triangles, expected.triangles, `${name} triangle count`);
  assert.ok(triangles < 6500, `${name} low-poly budget`);
  for (const part of contract.parts) assert.ok(semantic.has(part), `${name}/${part} required pivot`);
  for (const [part, position] of Object.entries(expected.attachments))
    near(semantic.get(part).getWorldPosition(new Vector3()).toArray(), position, `${name}/${part} attachment`);
