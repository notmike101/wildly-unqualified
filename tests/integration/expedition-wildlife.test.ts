...
import { distance, type Animal, type Vec3 } from '../../src/shared/shared.ts';
import {
    crew,
    command,
    walkTo,
...
for (const species of ['fox', 'owl', 'woodpecker', 'otter'] as const)
    test(`${species} leaves a disturbed action and retries locally after an ordinary whistle`, () => {
        ...
        if (species === 'otter') {
            ...
            const pool = run.world.waters.find((w) => water.every((v, index) => v >= w.min[index] && v <= w.max[index]),
            )!;
...
function observeFamily(t: TestContext, species: 'fox' | 'squirrel' | 'otter') {
    ...
    const input = (x: number, z: number, isCrouch: boolean) => applyCommand(run, observer.id, {
        worldId: run.worldId,
        type: 'input',
        value: {
    ...
    for (let index = 0; index < 90 * 60; index++) {
        input(0, 0, true);
        advanceRun(run, DT);
        samples.push({
            tick: run.tick,
    ...
    const behaviors = samples.map((s) => s.behavior);
    const displacements = samples.map((s) => distance(s.position, samples[0].position),
    );
    const heights = samples.map((s) => s.position[1]);
    ...
    t.diagnostic(
        JSON.stringify({
            ...
            displacement: Math.max(...displacements),
...
/**
 * Check that sampled movement stays within the resident bounds.
 *
 * @param samples Observed poses and behavior states.
 * @param animal Resident whose movement is being checked.
 * @param home Resident home position used to bound roaming.
 */
function bounded(samples: Sample[], animal: Animal, home: Vec3) {
    for (let index = 1; index < samples.length; index++) {
        assert.ok(
            distance(samples[index - 1].position, samples[index].position) <= 0.2,
            `${animal.id} jumped more than 20 cm in one production tick`,
        );
        assert.ok(
            distance(samples[index].position, home) < 35,
...
test('fox traverses its generated ground passage and lands a local pounce', (t) => {
    ...
    assert.ok(
        passages.filter((a) => samples.some((s) => distance(s.position, a.point) < 1),
        ).length >= 2,
    ...
    assert.ok(airborne !== -1, 'pounce has actual root lift');
    assert.ok(
...
test('otter enters its pond at a floating datum then returns to bank grooming', (t) => {
    ...
    const pool = run.world.waters.find((w) => water.point.every((n, axis) => n >= w.min[axis] && n <= w.max[axis]),
    )!;
...
test('repeated raccoons keep separate bodies while approaching shared woodland sites', () => {
    ...
    for (let tick = 0; tick < 120 * 60; tick++) {
        ...
        for (let index = 0; index < residents.length; index++)
            for (let index_ = index + 1; index_ < residents.length; index_++)
                assert.ok(
                    distance(
                        residents[index].pose.position,
                        residents[index_].pose.position,
                    ) >= 0.45,
                    `shared-site overlap at tick ${run.tick}: ${residents[index].id}/${residents[index_].id}`,
                );
    }
});
