...
test("portable release contains the exact runtime lock and no credentials or development sources", async (t) => {
  ...
  assert.deepEqual(
    (
      await readdir(join(destination, "wildly-unqualified"), {
        recursive: true,
      })
    )
  ...
  assert.equal(
    ...
    await readFile(join(source, "dist/index.html"), "utf8"),
  ...
  assert.deepEqual(
    ...
    await readFile(join(source, "dist/models/forest-kit-v3.glb")),
  ...
  assert.equal(
    entries.some((p) =>
      /(?:^|[\\/])(?:node_modules|data|data-expedition|tests|scripts|client|room\.json|run\.json|assets[\\/]author\.py)(?:$|[\\/])/.test(
        p,
      ),
    ),
    false,
