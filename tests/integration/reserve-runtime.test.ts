...
test('native generated crossings use their own seat and compound plank body', async () => {
    ...
    const items1 = world.fixtures.filter((f) => f.kind === 'crossing');

    for (const crossing of items1) {
        ...
        const properties: FieldProperty[] = world.props.map((p) => ({
            ...p,
        ...
        properties.find((p) => p.id === crossing.plankId)!.pose
            = structuredClone(at);
        ...
        try {
            ...
            for (let index = 0; index < 180; index++) result = physics.step(1 / 60);
            assert.ok(
...
test('schema 3 saves preserve exact blueprint and reject cross-world frozen references', async () => {
    ...
    const direction = await mkdtemp(path.join(tmpdir(), 'wu-schema3-'));
    ...
    try {
        ...
        const response1 = await readFile(path.join(direction, 'run.json'), 'utf8');

        assert.equal(response1.match(/"placements":/g)?.length, 1);
        const invalid = structuredClone(run);

        invalid.pendingPhotos[frame.id].worldId = 'another-world';
        await assert.rejects(saveRun(direction, invalid, new Map()), /world/i);
        invalid.pendingPhotos[frame.id].worldId = run.worldId;
        invalid.animals[0].species = 'mallard';
        await assert.rejects(saveRun(direction, invalid, new Map()), /identit/i);
        await writeFile(path.join(direction, 'run.json'), '{"version":2}');
        await assert.rejects(loadRun(direction), /version/i);
        assert.equal(
            await readFile(path.join(direction, 'run.json'), 'utf8'),
            '{"version":2}',
        );
    } finally {
        await rm(direction, { recursive: true, force: true });
    }
});
...
test('a seated generated crossing is permanent and cannot be recovered as loose equipment', async () => {
    ...
    const items2 = run.world.fixtures.filter((f) => f.kind === 'crossing');

    for (const fixture of items2) {
