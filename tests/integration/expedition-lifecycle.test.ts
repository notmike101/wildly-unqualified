...
test('a real climbing capture and resident phase survive pause, save, disconnect and rejoin', async (t) => {
    ...
    const direction = await mkdtemp(path.join(tmpdir(), 'wu-routine-rejoin-'));

    t.after(async () => {
        assert.equal(path.dirname(path.resolve(direction)), path.resolve(tmpdir()));
