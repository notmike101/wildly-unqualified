...
import type { ServerMessage } from '../../src/shared/shared.ts';
import {
    loadRoom,
    loadRun,
    saveRun,
...
test('only explicit equipment contacts and native tin contacts route to impact cues', () => {
    const eventCueKind = Server.eventCueKind;
...
/**
 * Start an isolated server with cleanup owned by the test.
 *
 * @param t Test context that owns cleanup and diagnostics.
 * @returns The server and its temporary web and private data paths.
 */
async function fixture(t: TestContext) {
    const root = await mkdtemp(path.join(tmpdir(), 'wu sockets '));
    const webDirectory = path.join(root, 'web');
    const dataDirection = path.join(root, 'private');
    ...
    return { ...server, root, webDir: webDirectory, dataDir: dataDirection };
}
...
test('only the compiled web directory and public readiness are served', async (t) => {
    ...
    await writeFile(path.join(server.dataDir, 'outside.js'), 'private');
    await symlink(server.dataDir, path.join(server.webDir, 'escape'), 'junction');
    for (const path of [
        '/room.json',
        '/server.ts',
        '/data/room.json',
        ...
        const response2 = await fetch(server.url + path);

        assert.ok((response2).status >= 400, path);
    }
});

/**
 * Submit a room admission request.
 *
 * @param url HTTP address of the test server.
 * @param secret Room admission secret submitted by the client.
 * @param name Display name or named scene node to select.
 * @param cookie Existing session cookie, when available.
 * @param requestOrigin Origin header used for the admission or socket request.
 * @returns The HTTP response, parsed identity and session cookie.
 */
...
/**
 * Open a socket and collect messages received from the server.
 *
 * @param url HTTP address of the test server.
 * @param cookie Existing session cookie, when available.
 * @param requestOrigin Origin header used for the admission or socket request.
 * @returns The socket and ordered received messages.
 */
...
/**
 * Encode a command using the negotiated world identity.
 *
 * @param client Connected socket and its received messages.
 * @param value Input payload under test.
 * @returns Serialized command ready for the socket.
 */
function encode(
 ...
 */
async function until(isPredicate: () => boolean, ms = 3000) {
    const end = Date.now() + ms;

    while (!isPredicate() && Date.now() < end)
        await new Promise((resolve) => setTimeout(resolve, 20));
    assert.ok(isPredicate(), 'Expected socket event before timeout');
}
...
test('nearby friends hear distinct whistle and rattle cues once, without reconnect or restore replay', async (t) => {
    ...
    await until(() => second.messages.some((m) => m.type === 'cue' && m.kind === 'whistle'),
    );
    assert.equal(
    ...
    for (let index = 0; index < 60; index++) {
        const state = first.messages
                .findLast((m) => m.type === 'snapshot')!.value,
            p = state.players.find((p) => p.id === host.identity.playerId)!,
    ...
    await until(
        () => first.messages.findLast((m) => m.type === 'snapshot')!.value.tin
            .holder === host.identity.playerId,
    );
    await until(
        () => first.messages.findLast((m) => m.type === 'snapshot')!.value.tick
            > 200,
        5000,
    );
    first.socket.send(encode(first, { type: 'use', seq: seq++ }));
    await until(() => second.messages.some((m) => m.type === 'cue' && m.kind === 'rattle'),
    );
    first.socket.send(encode(first, { type: 'start', seq: seq++ }));
    first.socket.send(encode(first, { type: 'photo', seq }));
    await until(() => second.messages.some((m) => m.type === 'cue' && m.kind === 'shutter'),
    );
    ...
    await until(
        () => first.messages
            .findLast((m) => m.type === 'snapshot')!
            .value.players.find((p) => p.id === guest.identity.playerId)!
            .connected === false,
    );
...
test('real sockets require origin and session, share movement, and reserve four slots', async (t) => {
    ...
    const credentials = JSON.parse(
        await readFile(path.join(server.dataDir, 'room.json'), 'utf8'),
    );

    const response3 = await fetch(server.url + '/api/session');

    assert.equal((response3).status, 401);
    const response4 = await admit(server.url, 'wrong');

    assert.equal((response4).response.status, 403);
    const response5 = await fetch(server.url + '/api/join', {
        method: 'POST',
        headers: {
    ...
    const initial = second.messages
        .findLast((m) => m.type === 'snapshot')!
        .value.players.find((p) => p.id === host.identity.playerId)!
        .position[0];
    ...
    await until(() => second.messages.some(
        (m) => m.type === 'snapshot'
            && m.value.players.some(
                (p) => p.id === host.identity.playerId && p.lastSeq === 1,
            ),
    ),
    );
    ...
    for (let index = 0; index < 2; index++) {
        const response6 = await admit(server.url, credentials.joinSecret, 'Guest ' + index);
    ...
    const response7 = await admit(server.url, credentials.joinSecret, 'Fifth');

    assert.equal(
        (response7).response.status,
        409,
    );
    const response8 = await fetch(server.url + '/api/invite', {
    ...
    assert.equal(
        ((await invitation.json()) as { secret: string }).secret,
...
test('websocket upgrades reject foreign origins, missing sessions, duplicates and oversized messages', async (t) => {
    ...
    /**
     * Attempt a socket handshake that must be rejected.
     *
     * @param cookie Existing session cookie, when available.
     * @param requestOrigin Origin header used for the admission or socket request.
     * @returns The rejected handshake status.
     */
    async function rejected(cookie: string, requestOrigin = origin) {
        return new Promise<number>((resolve, reject) => {
            ...
            ws.on('unexpected-response', (_request, resource) => {
                resource.resume();
                resolve(resource.statusCode!);
                ws.terminate();
            });
            ws.on('error', () => {});
            ws.on('open', () => {
...
test('zero sockets stay paused and a missing established world never silently resets', async (t) => {
    ...
    const response9 = await admit(server.url, room.hostSecret, 'Host');

    assert.equal(
        (response9).response.status,
        200,
    );
    assert.equal((await loadRun(server.dataDir))!.run.paused, true);
    await server.close();
    await rm(path.join(server.dataDir, 'run.json'));
    await rm(path.join(server.dataDir, 'run.json.bak'), { force: true });
...
test('failed final save keeps the server available and permits a repaired retry', async (t) => {
    ...
    await mkdir(path.join(server.dataDir, 'run.json.tmp'));
    await assert.rejects(server.close());
    const response10 = await fetch(server.url + '/healthz');

    assert.equal((response10).status, 200);
    await rm(path.join(server.dataDir, 'run.json.tmp'), { recursive: true });
    await server.close();
    assert.equal((await loadRun(server.dataDir))!.run.paused, true);
});
...
test('authenticated favorites stay shared in the ended exhibition and across restart', async (t) => {
    ...
    await until(() => [first, second].every((client) => client.messages.some((m) => m.type === 'snapshot'),
    ),
    );
    first.socket.send(
        encode(first, {
    ...
    await until(() => first.messages.some(
    ...
    await until(() => restarted.messages.some(
        (m) => m.type === 'snapshot' && m.value.album[0].favorites.length === 1,
    ),
    );
    assert.deepEqual(
        restarted.messages.findLast((m) => m.type === 'snapshot')!.value
            .album[0].favorites,
...
test('restart and relocation retain rule-earned photo credit, pending capture, bait and animals across a new origin with recovered host and reassigned guest', async (t) => {
    ...
    assert.equal(
        restored.animals.find((a) => a.id === subject.id)!.remaining,
        4.25,
    );
    ...
    assert.deepEqual(((await invitation.json()) as { pending: { playerId: string; name: string }[] }).pending, [
        { id: pending.identity.playerId, name: 'New guest' },
    ]);
    const reassign = async (cookie: string, slotId: string) => fetch(server.url + '/api/reassign', {
        method: 'POST',
        headers: {
            Origin: origin,
            Cookie: cookie,
    ...
    const response11 = await reassign(pending.cookie, 'guest');

    assert.equal((response11).status, 403);
    const response12 = await reassign(host.cookie, 'host');

    assert.equal((response12).status, 409);
    const originalGuest = connect(server.url, oldGuestCookie);

    await until(() => originalGuest.messages.some((m) => m.type === 'welcome'));
    const response13 = await reassign(host.cookie, 'guest');

    assert.equal((response13).status, 409);
    originalGuest.socket.terminate();
    await until(
        () => client.messages
            .findLast((m) => m.type === 'snapshot')!
            .value.players.find((p) => p.id === 'guest')!.connected === false,
    );
    const privateRoom = await readFile(path.join(dataDirectory, 'room.json'), 'utf8');

    await writeFile(
        path.join(dataDirectory, 'room.json'),
    ...
    for (let index = 0; index < 100; index++) {
        try {
            await stat(path.join(dataDirectory, 'room.json.tmp'));
            isSaving = true;
            break;
        } catch {
    ...
    assert.ok(isSaving, 'Reassignment reached its real filesystem write');
    ...
    const response14 = await reassignment;

    assert.equal((response14).status, 200);
    const response15 = await fetch(server.url + '/api/session', {
        headers: { Cookie: oldGuestCookie },
    });
    ...
    assert.equal(((await guestIdentity.json()) as { playerId: string }).playerId, 'guest');
    ...
    const favorite = (seq: number, isSelected: boolean, photoId = frame.id) => ({
        type: 'favorite',
        seq,
        photoId,
        selected: isSelected,
    });
    const latest = () => guestClient.messages.findLast((m) => m.type === 'snapshot')!.value;

    client.socket.send(
        encode(client, { ...favorite(2, true), playerId: 'guest' }),
    );
    await until(() => client.messages.some(
    ...
    await until(() => client.messages.some((m) => m.type === 'notice' && /Replayed/.test(m.text)),
    );
    ...
    await until(
        () => latest().players.find((p) => p.id === 'guest')!.lastSeq === 2,
    );
    ...
    const upload = (cookie: string, bytes: Buffer, type = 'image/jpeg') => fetch(server.url + '/api/photos/' + frame.id, {
        method: 'POST',
        headers: { Origin: origin, Cookie: cookie, 'Content-Type': type },
        body: new Uint8Array(bytes),
    });

    const response16 = await upload(pending.cookie, jpeg);

    assert.equal((response16).status, 403);
    const response17 = await upload(host.cookie, jpeg, 'image/png');

    assert.equal((response17).status, 415);
    const response18 = await upload(host.cookie, Buffer.alloc(65_537));

    assert.equal((response18).status, 413);
    const response19 = await upload(host.cookie, Buffer.from('invalid'));

    assert.equal((response19).status, 400);
    const response20 = await upload(host.cookie, jpeg);

    assert.equal((response20).status, 200);
    const response21 = await upload(host.cookie, jpeg);

    assert.equal((response21).status, 200);
    ...
    const movedDirection = path.join(root, 'copied private data');

    await cp(dataDirectory, movedDirection, { recursive: true });
    ...
    server = await startServer({
        ...
        port: movedPort,
        origin: movedOrigin,
        dataDir: movedDirection,
        webDir: webDirectory,
    });
    assert.notEqual(server.url, oldURL);
    assert.equal(server.url, movedOrigin);
    const response22 = await admit(server.url, room.hostSecret, 'Old-origin request');
    ...
    const response23 = await fetch(server.url + '/api/session', {
        headers: { Cookie: pending.cookie },
    });
    ...
    const saved = (await loadRun(movedDirection))!;
...
test('authenticated join negotiates one hashed world before state and rejects stale input', async (t) => {
    ...
    assert.equal(client.messages[2].type, 'snapshot');
    ...
    await until(() => client.messages.some(
        (m) => m.type === 'snapshot'
            && m.value.players.some((p) => p.lastSeq === 1),
    ),
    );
