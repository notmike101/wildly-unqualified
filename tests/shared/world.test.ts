...
test('generated reserves contain reachable diverse commissioned wildlife', (t) => {
    ...
    for (const seed of [
        ...
        1,
        0xFF_FF_FF_FF,
        ...Array.from({ length: 256 }, (_, index) => index * 65_537),
    ]) {
    ...
    assert.deepEqual(
        [...usedSpecies].toSorted((a, b) => a.localeCompare(b)),
        RESERVE_SPECIES.toSorted((a, b) => a.localeCompare(b)),
    );
    ...
    const uniqueSeeds = new Set(metrics.map((m) => m.seed)).size;

    t.diagnostic(
        JSON.stringify({
            entries: metrics.length,
            uniqueSeeds,
            requiredSpecies: [...usedSpecies].toSorted((a, b) => a.localeCompare(b)),
            models,
...
test('full uint32 seed changes layout and objective bindings', () => {
    const worlds = [0, 0x80_00_00_00, 0xFF_FF_FF_FF, 0x7F_FF_FF_FF].map((seed) => generateReserve(seed, 'diversity'),
    );
...
test('required subjects have clear production camera rays and distinct resident positions', async () => {
    ...
    const items1 = w.commissions.filter((c) => c.required);

    for (const c of items1) {
...
test('water has a submerged physical bed and one realizable plank seat', () => {
    ...
    const items2 = w.fixtures.filter((f) => f.kind === 'crossing');

    for (const f of items2) assert.equal(Object.keys(f.seats).length, 1);
});
...
test('small residents have close camera positions and physically supported perches', async () => {
    ...
    const items3 = w.residents.filter((r) => ['rabbit', 'squirrel', 'otter', 'woodpecker', 'mallard', 'owl'].includes(
        r.species,
    ),
    );
    ...
    for (const r of items3) {
        assert.ok(
            reserveApproaches(w, r.home).some(
                (n) => Math.hypot(n.position[0] - r.spawn[0], n.position[2] - r.spawn[2])
                    <= 6,
            ),
            r.id,
        );
    ...
    for (
        ...
        for (const pocket of w.pockets) {
            ...
            if (p) {
                ...
                const items4 = pocket.anchors.filter(
                    (a) => a.kind === 'perch' && !a.id.includes('woodpecker'),
                );

                for (const a of items4) {
...
test('review: actual camp tin and prop origins require ground support', () => {
    ...
    for (const mutate of [
        ...
        ...original.props.map((_, index) => (w: typeof original) => {
            w.props[index].pose.position[1] = 30;
        }),
    ]) {
        const w = structuredClone(original);
...
test('review: nested geometry is exact typed and matches its fixture', () => {
    ...
    const items5 = mutations.entries();

    for (const [index, mutate] of items5) {
        const w = structuredClone(original);

        mutate(w);
        assert.throws(
            () => validateReserve(w),
            `nested geometry mutation ${index}`,
        );
    }
});
