...
import * as Shared from '../../src/shared/shared.ts';
import {
...
/**
 * Supply a test world identity before parsing a client message.
 *
 * @param value Input payload under test.
 * @returns The parsed command or rejection result.
 */
...
const equipmentTarget = Shared.equipmentTarget;
const equipmentUseTarget = Shared.equipmentUseTarget;
const playerSpeed = Shared.playerSpeed;
const propertyBoxes = Shared.propertyBoxes;
const player: Player = {
    id: 'p1',
    name: 'Player',
    slot: 0,
...
test('equipment selectors choose only visible free handles for E and free bait cups for Q', () => {
    ...
    const item = property('case', [0, 1, -1]);
    ...
    const decoy = property('decoy', [0, 1, -1.7]),
        sideCase = property('case', [1.5, 1, -0.2]),
        decoyDefinition = {
            ...definition,
    ...
    const heldScreen = property('screen', [10, 1, 10]);
...
test('compound prop boxes preserve openings and carry speed follows holder count', () => {
    ...
    const item = property('case', [3, 2, 4]);
...
test('wire commands require a bounded exact world identity before gameplay validation', () => {
    ...
    // eslint-disable-next-line unicorn/no-null -- Exercise explicit null as malformed protocol input.
