...
const PLANK_PLACEMENTS = Level.PLANK_PLACEMENTS;
const PROP_DEFINITIONS = Level.PROP_DEFINITIONS;
...
/**
 * Walk a player through route waypoints using collision checks.
 *
 * @param points Ordered world positions to traverse.
 * @param surfaces Walkable surfaces used for movement support.
 * @returns The final player state.
 */
function walk(points: Vec3[], surfaces = WALKABLES) {
    ...
    const items1 = points.slice(1);

    for (const target of items1) {
        for (
            let step = 0;
