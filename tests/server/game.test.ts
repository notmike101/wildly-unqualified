const pairPoint = (x: number, y: number, z: number) => photoPoint(78 - x, y, z);
...
const crossing = (run: ReturnType<typeof createRun>) => run.world.fixtures.find(
    (f) => f.plankId === run.props.find((p) => p.kind === 'plank')!.id,
)!;
const seat = (run: ReturnType<typeof createRun>) => Object.values(crossing(run).seats)[0];

/**
 * Locate a resident activity anchor within its home pocket.
 *
 * @param run Mutable simulation state for this scenario.
 * @param species Species whose resident is selected.
 * @param kind Required fixture or behavior kind.
 * @returns The matching activity anchor position.
 */
function anchor(
...
function crew() {
    ...
    advanceRun(run, 1 / 60);
    for (let index = 0; index < 6; index++) advanceRun(run, 1 / 60);
...
test('an open case spills one recoverable portion on a real sharp carry turn, with a two-second guard', () => {
    ...
    assert.ok(
        WALKABLES.some(
            (s) => surfaceHeight(s, pile.position[0], pile.position[2])
                === pile.position[1],
        ),
    );
...
test('closed turns and slow open carrying do not spill; a case bump does', () => {
    ...
    item.pose = pose([0, 0.325, 0]);
    p.position = [-0.75, 0, 1];
    run.props.find((property) => property.kind === 'screen')!.pose = pose([
        3, 0.97, 0,
    ]);
    command(run, 'a', 'interact');
    ...
    for (let index = 0; index < 30; index++) move(0, 0.2);
    assert.equal(run.spills.length, 0);
    command(run, 'a', 'use');
    for (let index = 0; index < 10; index++) move(1, 0.2);
    assert.equal(run.spills.length, 0, 'ordinary slow carry is harmless');
    for (let index = 0; index < 300; index++) move(1, 0.2);
    assert.ok(run.events.some((error) => error.kind === 'impact'));
    assert.equal(
        run.spills.length,
        1,
...
test('spills stay bounded, expire, and depleted camp supplies can be restored with the tin', () => {
    ...
    command(run, 'a', 'interact');
    command(run, 'a', 'use');
    run.spills = Array.from({ length: 8 }, (_, index) => ({
        id: `old-${index}`,
        position: [10 + index, 0, 0] as Vec3,
        portions: 1,
...
/**
 * Advance the simulation at sixty ticks per second.
 *
 * @param run Mutable simulation state for this scenario.
...
/**
 * Place the open tin at the requested photo position.
 *
 * @param run Mutable simulation state for this scenario.
 * @param point World position to place the fixture.
 */
...
/**
 * Translate reference coordinates into the generated photo pocket.
 *
 * @param x Horizontal coordinate in the reference photograph.
 * @param y Vertical coordinate in the reference photograph.
 * @param z Depth coordinate in the reference photograph.
 * @returns The translated world position.
 */
...
/**
 * Create a photo world with optional additional occluders.
 *
 * @param boxes Additional occluders included in the photo world.
 * @returns The world geometry used for photo evaluation.
 */
...
/**
 * Build a frozen pair photograph with controlled subject poses.
 *
 * @returns The frozen photo frame.
 */
...
test('a prop body occludes its opposite handle without hiding a visible near handle', () => {
    const run = crew(),
        item = run.props.find((property) => property.kind === 'case')!;

    run.tin.pose.position = [50, 1, 50];
    const loopItems1 = run.props
        .filter((property) => property !== item)
        .entries();

    for (const [index, property] of loopItems1) {
        property.pose.position = [50 + index * 3, 1, 50];
    }
    item.pose.position = [0, 0.325, 0];
    run.players[1].position = [-0.75, 0, 1];
    command(run, 'b', 'interact');
...
test('plank seating, gate latch, case stock and local prop recovery update authoritative state', () => {
    const run = crew(),
        p = run.players[0],
        plank = run.props.find((property) => property.kind === 'plank')!,
        item = run.props.find((property) => property.kind === 'case')!;
    ...
    const parked = run.props
        .filter((property) => property !== plank)
        .map((property) => [property, structuredClone(property.pose)] as const);

    const loopItems2 = parked.entries();

    for (const [index, [property]] of loopItems2) {
        property.pose.position = [50 + index * 3, 1, 50];
    }
    command(run, 'a', 'interact');
    assert.equal(run.route.gate.open, true);
    for (const [property, saved] of parked) {
        property.pose = saved;
    }
...
test("ordinary recovery rescues the browser's tipped decoy and permits native carrying without losing progress", async () => {
    const run = crew(),
        fern = run.players[1],
        decoy = run.props.find((property) => property.kind === 'decoy')!,
        plank = run.props.find((property) => property.kind === 'plank')!;
    ...
    assert.ok(
        run.world.stations.some(
            (s) => distance(recovered, [s.recover[0], s.recover[1] + 0.5, s.recover[2]])
                < 1e-6,
        ),
    );
    // eslint-disable-next-line unicorn/no-null -- The serialized game contract represents an empty slot with null.
...
test('recovery preserves modest equipment tilt, pure yaw, and held or seated equipment', () => {
    for (const kind of ['case', 'decoy', 'screen', 'plank'] as const) {
        const loopItems3 = kind === 'plank'
            ? ['bank tilt', 'yaw', 'held', 'seated']
            : ['bank tilt', 'yaw', 'held'];

        for (const condition of loopItems3) {
            const run = crew(),
                property = run.props.find((property) => property.kind === kind)!;
          ...
          condition === 'yaw'
            ...
            if (condition === 'held') {
                property.pose.rotation = [1, 0, 0, 0];
                property.holders[0] = 'b';
            } else if (condition === 'seated') {
                property.pose = structuredClone(seat(run));
                property.placed = true;
                run.route[crossing(run).id] = { open: true, seat: 'left' };
            }
            run.tin.holder = 'a';
            ...
            assert.deepEqual(property, before, `${kind}: ${condition}`);
        }
    }
});
...
test('disconnect releases an equipment handle while the other holder can continue', () => {
    const run = crew(),
        item = run.props.find((property) => property.kind === 'screen')!;
...
test('two holders retain exact handles across a three dimensional slope', () => {
    ...
    const retained = plank.holders.flatMap((id, handleIndex) => (id
        ? [
                distance(
                    propertyPoint(
                        PROP_DEFINITIONS.plank.handles[handleIndex],
                        plank.pose,
                    ),
                    run.players
...
test('player separation never pushes a supported body into the washout', () => {
    ...
    assert.ok(
        [
            ...
            (surface) => surfaceHeight(surface, a.position[0], a.position[2]) !== undefined,
        ),
...
test('a released player can move out of an existing equipment overlap', () => {
    ...
    const loopItems4 = run.props
        .filter((property) => property !== item)
        .entries();

    for (const [index, property] of loopItems4) {
        property.pose.position = [50 + index * 3, 1, 50];
    }
    item.pose = {
...
test('crew slots and seeded world identity survive reconnection', () => {
    ...
    assert.throws(() => createRun(0x1_00_00_00_00), /uint32/i);
});
...
test('rattle brings inspection before theft; whistle diverts the same carried tin and it reaches the stash', () => {
    ...
    for (
        let index = 0;
        index < 80 && run.tin.holder !== `animal:${r.id}`;
        index++
    )
        step(run, 0.1);
...
test('heron consumes bait once, retreats visibly from noise, then returns and displays after quiet', () => {
    ...
    for (let index = 0; index < 150 && h.behavior !== 'display'; index++)
        step(run, 0.1);
    ...
    for (let index = 0; index < 350 && h.behavior !== 'display'; index++)
        step(run, 0.1);
...
test('photo geometry checks front, frame clipping, subject size, occlusion and actual behavior', () => {
    ...
    const wallMin = photoPoint(32, 0, -27);
    const wallMax = photoPoint(41, 4, -26);
    ...
    const equipmentBlocked = structuredClone(frame),
        fieldCase = equipmentBlocked.props.find(
...
test('each selected assignment, camp return and connected readiness gate the exhibition', () => {
    ...
    const items1 = run.world.commissions
        .filter((c) => c.required)
        .map((c) => c.id);

    for (const missing of items1) {
...
test('bait refills and contextual recovery preserve progress; camp ending requires every connected player ready', () => {
    const run = crew(),
        ...
        patchId = run.world.pockets
            .find(
                (p) => p.id === run.world.residents.find((r) => r.species === 'heron')!.home,
            )!
            .anchors.find((a) => a.kind === 'feed')!.id;
...
test('a linked generated encounter preserves tin identity through theft, pause, recovery and pair credit', () => {
    ...
    const tin = run.tin,
        ...
        a = run.players[0],
        b = run.players[1];
    const until = (isCondition: () => boolean, seconds: number) => {
        for (let index = 0; index < seconds * 10 && !isCondition(); index++)
            step(run, 0.1);
        assert.ok(isCondition(), JSON.stringify({ r, h, tin }));
    };
...
test("a local plank cannot bypass another crossing's water or leave its own deck strip", () => {
    for (const variant of ['wrong-plank', 'outside-strip', 'rotated'] as const) {
        ...
        assert.ok(
            run.events.some((error) => error.kind === 'impact'),
            variant,
        );
    }
});
