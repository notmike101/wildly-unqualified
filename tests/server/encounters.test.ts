...
import { saveRun, loadRun } from '../../src/server/persistence/save.ts';
...
function step(run: ReturnType<typeof createRun>, seconds: number) {
    for (let index = 0; index < seconds * 60; index++) advanceRun(run, 1 / 60);
}
        ...
        for (const open of [false, true]) {
            ...
            if (open)
              ...
              fixture.kind === 'crossing'
                  ...
                  // eslint-disable-next-line unicorn/no-null -- The route contract uses null for an unseated fixture.
                  : null,
                    };
            const route = run.route;

            const items1 = run.world.navNodes.filter((n) => n.id.endsWith('camera-a'),
            );

            for (const node of items1) {
                ...
                for (const to of path) {
                    ...
                    for (let index = 0; index <= steps; index++) {
                        const point = from.map(
                            (v, a) => v + ((to[a] - v) * index) / Math.max(1, steps),
                        ) as Vec3;
                        const heights = [
                            ...run.world.walkables,
...
test('an expired running input cannot manufacture continuing noise', () => {
    ...
    step(run, 1);
    const lastNoise = run.events.findLast((error) => error.kind === 'noise')!;
...
test('unreachable lures behind equipment never pull an animal through a panel', () => {
    ...
    for (let index = 0; index < 80; index++) {
        ...
        step(run, 0.1);
        const items2 = propertyBoxes(screen, PROP_DEFINITIONS.screen);
...
test('an inaccessible tin does not hide an accessible filled decoy cup', () => {
    ...
    decoy.open = true;
    for (let index = 0; index < 60 && r.behavior !== 'investigate'; index++)
        step(run, 0.1);
...
test('calm goal choices avoid immediately repeating a completed goal', () => {
    ...
    const items3 = run.animals.filter((a) => ['raccoon', 'deer', 'heron'].includes(a.species),
    );

    for (const animal of items3) {
        const memory = run.animalMemory[animal.id];

        assert.ok(memory.recentGoals.length >= 2);
        const goals = [...memory.recentGoals, memory.goal];

        assert.ok(goals.every((g, index) => !index || g !== goals[index - 1]));
    }
});
...
test('supported generated legacy behavior commissions arise from real encounter updates', () => {
    for (const [seed, species, behavior] of [
        ...
        for (let index = 0; index < 200 && animal.behavior !== behavior; index++)
            step(run, 0.1);
...
test('saved encounter memory is bounded, exact, referenced and preserves the seeded configuration', async (t) => {
    const direction = await mkdtemp(path.join(tmpdir(), 'wu encounters '));
    ...
    const loaded = (await loadRun(direction))!.run;

    assert.deepEqual(loaded.world, run.world);
    assert.deepEqual(loaded.animalMemory, run.animalMemory);
    for (const mutate of [
        (r: typeof run) => delete r.animalMemory[r.animals.find((a) => a.species === 'deer')!.id],
        (r: typeof run) => (r.animalMemory[
            r.animals.find((a) => a.species === 'deer')!.id
        ].recentGoals = Array.from({ length: 5 }, () => 'repeat')),
        (r: typeof run) => (r.animalMemory[
            r.animals.find((a) => a.species === 'deer')!.id
        ...
        mutate(invalid);
        await assert.rejects(saveRun(direction, invalid, new Map()));
    }
});
...
test('legacy residents retain individual calm homes instead of converging on one species point', () => {
    ...
    step(run, 40);
    for (let index = 0; index < deer.length; index++)
        for (let index_ = index + 1; index_ < deer.length; index_++) {
            assert.ok(
                distance(deer[index].pose.position, deer[index_].pose.position) > 0.5,
                `${deer[index].id} overlaps ${deer[index_].id}`,
            );
            assert.notDeepEqual(deer[index].target, deer[index_].target);
        }
});

test('an ordinary dropped hat stays on reachable ground through ticks, restore and recovery', async (t) => {
    const direction = await mkdtemp(path.join(tmpdir(), 'wu-ground-hat-'));
