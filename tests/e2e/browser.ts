...
const output = pathModule.resolve('.artifacts/wildly-unqualified/mvp-2026-09-05');
...
const resumedFrom = process.env.WU_RESUME_FROM
    ? pathModule.resolve(process.env.WU_RESUME_FROM)
    : undefined;

assert.ok([2, 4].includes(count) && [0, 150].includes(latency));
const evidence = pathModule.resolve(output, `outing-${count}p-${latency}ms-${Date.now()}`);

await mkdir(evidence, { recursive: true });
await copyFile(
    fileURLToPath(import.meta.url),
    pathModule.resolve(evidence, 'outing-driver.ts'),
);
await cp(
    resumedFrom && process.env.WU_RESUME_CURRENT_RUNTIME !== '1'
        ? pathModule.resolve(resumedFrom, 'web')
        : fileURLToPath(new URL('../../dist', import.meta.url)),
    pathModule.resolve(evidence, 'web'),
    {
        recursive: true,
        errorOnExist: true,
        force: false,
...
const runtime = pathModule.resolve(evidence, 'runtime');
let serverEntry = pathModule.resolve(runtime, 'src/server/server.ts');

if (resumedFrom && process.env.WU_RESUME_CURRENT_RUNTIME !== '1') {
    await cp(pathModule.resolve(resumedFrom, 'runtime'), runtime, {
        recursive: true,
        errorOnExist: true,
        force: false,
    });
    try {
        ...
        serverEntry = pathModule.resolve(runtime, 'server.ts');
        await access(serverEntry);
    }
} else {
...
if (resumedFrom)
    await cp(pathModule.resolve(resumedFrom, 'data'), pathModule.resolve(evidence, 'data'), {
        recursive: true,
        errorOnExist: true,
        force: false,
    });
...
const serverOptions = {
    ...
    webDir: pathModule.resolve(evidence, 'web'),
    dataDir: pathModule.resolve(evidence, 'data'),
};
const owned = { server: await startServer(serverOptions), isRestarting: false };
...
if (resumedFrom) {
    ...
    try {
        priorText = await readFile(pathModule.resolve(resumedFrom, 'failure.json'), 'utf8');
    } catch {
        priorText = await readFile(pathModule.resolve(resumedFrom, 'progress.json'), 'utf8');
    }
...
async function checkpoint(name: string) {
    ...
    await writeFile(
        pathModule.resolve(evidence, 'progress.json'),
        JSON.stringify({ coverage, log, state }, undefined, 2),
    );
    await pages[0].screenshot({ path: pathModule.resolve(evidence, name + '.png') });
    console.log(
...
async function habitatViews(name: string, center: Vec3) {
    ...
    await p.screenshot({ path: pathModule.resolve(evidence, name + '-first-person.png') });
    await aim(p, [position[0], position[1] + 9, position[2] - 6]);
    await p.screenshot({ path: pathModule.resolve(evidence, name + '-upward-sky.png') });
    await aim(p, [center[0], 1.1, center[2]]);
}
...
async function restart(label: string) {
    ...
    owned.isRestarting = true;
    await owned.server.close();
    await sleep(3600);
    owned.server = await startServer(serverOptions);
    for (const p of pages)
        await p.waitForFunction(
            () => globalThis.window.wildly.snapshot?.players.find(
                (p) => p.id === globalThis.window.wildly.playerId,
            )?.connected
    ...
    if (before.phase !== 'exhibition') {
        ...
        await pages[0].waitForFunction(() => !globalThis.window.wildly.snapshot!.paused);
    }
    const after = await snapshot(pages[0]);

    owned.isRestarting = false;
...
try {
    await runOuting({
         ...
         */
        isRestarting: () => owned.isRestarting,
    });
} catch (error) {
    const views = await Promise.all(pages.map(async (p) => {
        try {
            return await p.evaluate(() => ({
                ...
                assets: globalThis.window.wildly?.assetErrors,
                render: globalThis.window.wildly?.render,
                p95: globalThis.window.wildly?.p95,
            }));
        } catch (viewError) {
            return { error: String(viewError) };
        }
    }));

    // Explicit null preserves the failure evidence schema when a page cannot be read.
    // eslint-disable-next-line unicorn/no-null
    let failureSnapshot: Awaited<ReturnType<typeof snapshot>> | null = null;

    if (pages[0]) {
        try {
            failureSnapshot = await snapshot(pages[0]);
        } catch {
            // A missing snapshot must not replace the original outing failure.

        }
    }
    await writeFile(
        pathModule.resolve(evidence, 'failure.json'),
        JSON.stringify({ error: String(error), coverage, errors, network, views, log, snapshot: failureSnapshot }, undefined, 2),
    );
    try {
        await pages[0]?.screenshot({ path: pathModule.resolve(evidence, 'failure.png') });
    } catch {
        // Screenshots are best effort after a browser failure.
    ...
    const isConnectionsClosed = browsers.every((browser) => !browser.isConnected());

    await writeFile(
        pathModule.resolve(evidence, 'cleanup.json'),
        JSON.stringify(
            {
                connectionsClosed: isConnectionsClosed,
                ownedBrowserCount: browsers.length,
                results: cleanup.map((r) => (r.status === 'fulfilled' ? r.status : String(r.reason)),
                ),
            },
            undefined,
            2,
    ...
    const saved = JSON.parse(
        await readFile(pathModule.resolve(evidence, 'data/run.json'), 'utf8'),
    );
    ...
    for (const [id, bytes] of Object.entries(
        ...
        await writeFile(
            pathModule.resolve(evidence, `outing-${id}.jpg`),
            Buffer.from(bytes, 'base64'),
        );
    }
    assert.ok(isConnectionsClosed, 'owned browser control connections are closed');
    assert.ok(
