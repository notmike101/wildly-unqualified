...
import { alignLocalCarry, loadAsset } from '../../src/client/rendering/view.ts';
...
test('predicted camera keeps carried geometry and its other holder together without changing authority', () => {
    ...
    const handle = () => propertyPoint(PROP_DEFINITIONS.case.handles[0], render.props[0].pose);
    ...
    const movedHandle = handle();

    assert.ok(
        Math.abs(distance(movedHandle, predicted.position) - originalOffset)
        < 1e-10,
    );
    assert.ok(
        Math.abs(
...
test('grazing and scanning photo points follow the actual imported deer rig', async () => {
    ...
    for (const behavior of ['graze', 'settle', 'alert'] as const) {
        ...
        for (const tick of [0, 81, 319]) {
            ...
            const expected = subjectPoints(animal, tick).map((p) => propertyPoint(p, animal.pose),
            );

            const items1 = ['PhotoBody', 'PhotoHead'].entries();

            for (const [index, part] of items1) {
...
test('a transient model failure can retry while concurrent and successful loads stay shared', async () => {
    ...
    loader.loadAsync = () => (++calls === 1
        ? Promise.reject(new Error('transient model failure'))
