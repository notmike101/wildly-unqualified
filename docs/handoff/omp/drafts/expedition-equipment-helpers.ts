...
import { advanceRun, type RunState } from "./game.ts";
import { animalRoute } from "./encounters.ts";
import { PROP_DEFINITIONS, fixtureBoxes } from "./level.ts";
import {
  distance,
  propPoint,
  propBoxes,
...
import { rotate, xyz } from "./wildlife.ts";
import {
  input,
  command,
  walkTo,
...
export function feedingSetup(run: RunState) {
  const c = run.world.commissions.find((c) => c.kind === "setup")!,
