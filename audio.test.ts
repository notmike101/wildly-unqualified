...
class Context {
  ...
  async decodeAudioData(bytes: ArrayBuffer) {
    if (Context.failDecode) throw Error("decode");
    return {
  ...
  async resume() {
    if (Context.failResume) throw Error("autoplay");
    this.state = "running";
  }
  async suspend() {
...
const effects = () =>
  Context.instances[0].sources.filter(
...
test("surface steps have different palettes and never immediately repeat", async () => {
  ...
  for (const material of ["trail", "wood", "wet"])
    for (let i = 0; i < 3; i++)
      a.cue(cue(`${material}${i}`, "footstep", { material }));
  await settle();
  const labels = effects().map((s) => s.buffer!.label);
...
test("effects cap includes loading reservations; ended voices free capacity", async () => {
  const a = setup();
  await a.unlock();
  for (let i = 0; i < 30; i++) a.cue(cue(`event${i}`, "whistle"));
  await settle();
...
test("latest water distance applies when an asynchronous bed finishes loading", async () => {
  ...
  await settle();
  const water = Context.instances[0].sources.find((s) =>
    s.buffer?.label.endsWith("water.wav"),
  )!;
...
test("delivered palette matches source licenses and hashes; original PCM has no clipped samples", () => {
  ...
  for (const asset of manifest.assets) {
    ...
    if (asset.url.endsWith(".wav")) {
      ...
      assert.equal(frames / 22050, asset.duration);
      decoded += frames * 4;
      let peak = 0;
      ...
      assert.equal(
        createHash("sha256")
          .update(readFileSync(new URL(`./${asset.source}`, import.meta.url)))
          .digest("hex"),
        asset.sourceSha256,
      );
    } else {
