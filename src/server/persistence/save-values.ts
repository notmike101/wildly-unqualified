 ...
 */
import type { Player, Animal, Tin, Vec3, FieldProperty, FixtureState, Spill, CrewHat, PhotoFrame } from '../../shared/shared.ts';
 ...
 */
function object(value: unknown, fields?: string[], optional?: string[]) {
 ...
 */
export function text(value: unknown, max = 128): asserts value is string {
...
export function integer(
    ...
    number_(value, min, max);
    if (!Number.isSafeInteger(value)) throw new Error('Invalid saved integer');
}
 ...
 */
export function bool(value: unknown): asserts value is boolean {
    if (typeof value !== 'boolean') throw new Error('Invalid saved boolean');
}
 ...
 */
export function one(value: unknown, choices: string[]): asserts value is string {
    if (typeof value !== 'string' || !choices.includes(value))
        throw new Error('Invalid saved enum');
}
 ...
 */
export function list<T>(value: unknown, max: number, check: (v: unknown) => asserts v is T): asserts value is T[] {
    if (!Array.isArray(value) || value.length > max)
        throw new Error('Invalid saved list');
    for (const element of value) check(element);
}
 ...
 */
function vector(value: unknown, size = 3): asserts value is number[] {
    if (!Array.isArray(value) || value.length !== size)
        throw new Error('Invalid saved vector');
    for (const component of value) number_(component, -10_000, 10_000);
}
 ...
 */
export function position(value: unknown): asserts value is Vec3 {
    if (!Array.isArray(value) || value.length !== 3)
        throw new Error('Invalid saved position');
    for (const component of value) number_(component, -512, 512);
}
...
function pose(value: unknown) {
    const v = object(value, ['position', 'rotation']);

    position(v.position);
    vector(v.rotation, 4);
    const n = Math.hypot(...v.rotation);

    if (n < 0.9 || n > 1.1) throw new Error('Invalid saved quaternion');
}
 ...
 */
export function player(value: unknown): asserts value is Player {
    const v = object(
        value,
        [
            'id',
    ...
    integer(v.slot, 0, 3);
    position(v.position);
    number_(v.yaw, -Math.PI * 4, Math.PI * 4);
    number_(v.pitch, -1.45, 1.45);
    integer(v.lastSeq);
    bool(v.connected);
    integer(v.inputTick);
 ...
 */
function animal(value: unknown): asserts value is Animal {
 ...
 */
export function tin(value: unknown): asserts value is Tin {
    const v = object(value, [
        'pose',
        'velocity',
        'angularVelocity',
 ...
 * blueprint.
 */
export function residentInventory(value: unknown, world: ReserveBlueprint): asserts value is Animal[] {
    ...
    if (
        ...
        || animals.some(
            (a) => world.residents.every((r) => !(r.id === a.id && r.species === a.species)),
        )
    )
        throw new Error('Invalid animal count or identities');
}
 ...
 */
export function carrier(value: unknown, ids: Set<string>, world: ReserveBlueprint) {
    if (value === null || (typeof value === 'string' && ids.has(value))) return;
    if (
        typeof value !== 'string'
        ...
        throw new Error('Missing saved tin holder');
}
 ...
 */
export function stateBounds(v: Pick<PhotoFrame, 'players' | 'animals' | 'tin' | 'props' | 'spills' | 'hats' | 'tick' | 'route'>, world: ReserveBlueprint) {
    const points = [
        ...v.players.map((p) => p.position),
        ...v.animals.flatMap((a) => [a.pose.position, a.target]),
        v.tin.pose.position,
        ...v.props.map((p) => p.pose.position),
        ...v.spills.map((s) => s.position),
        ...v.hats.map((h) => h.position),
    ];

    for (const point of points)
        if (
            [0, 2].some(
                (axis) => point[axis] < world.bounds.min[axis] - 2
                    || point[axis] > world.bounds.max[axis] + 2,
            )
            || point[1] < -8
            || point[1] > 64
        )
            throw new Error('Saved position outside world bounds');
}
...
function properties(
...
): asserts value is FieldProperty[] {
    const propertyKinds = new Map(world.props.map((p) => [p.id, p.kind]));
    ...
    list(value, world.props.length, (x) => {
        ...
        id(v.id);
        if (propertyKinds.get(v.id) !== v.kind) throw new Error('Invalid saved field prop');
        pose(v.pose);
        vector(v.velocity);
        vector(v.angularVelocity);
        if (!Array.isArray(v.holders) || v.holders.length !== 2)
            throw new Error('Invalid saved prop holders');
        for (const holder of v.holders)
            if (holder !== null && !playerIds.has(holder))
                throw new Error('Missing saved prop holder');
        bool(v.placed);
        bool(v.open);
        integer(v.spillUntilTick);
    });
    const values = value as FieldProperty[];

    if (
        values.length !== world.props.length
        || new Set(values.map((x) => x.id)).size !== world.props.length
    )
        throw new Error('Invalid saved field prop inventory');
}
 ...
 */
export function route(value: unknown, world: ReserveBlueprint): asserts value is FixtureState {
    const states = object(
        value,
...
export function plankRoute(
    propertyValues: FieldProperty[],
    routeValue: FixtureState,
    world: ReserveBlueprint,
) {
    for (const fixture of world.fixtures) {
        if (fixture.kind !== 'crossing') continue;
        const plank = propertyValues.find((p) => p.id === fixture.plankId),
            state = routeValue[fixture.id],
            expected = state.seat ? fixture.seats[state.seat] : undefined;
        ...
        if (
            ...
            throw new Error('Saved plank and crossing route disagree');
    }
}
 ...
 */
export function spills(value: unknown): asserts value is Spill[] {
    const seen = new Set<string>();

    list(value, 8, (x) => {
        const v = object(x, ['id', 'position', 'portions', 'untilTick']);

        id(v.id);
        if (seen.has(v.id)) throw new Error('Duplicate saved spill');
        seen.add(v.id);
        position(v.position);
...
export function hats(
    ...
    list(value, 4, (x) => {
        ...
        if (!playerIds.has(v.owner) || owners.has(v.owner))
            throw new Error('Invalid saved hat owner');
        owners.add(v.owner);
        one(v.carrier, [
            'owner',
            'ground',
    ...
    if (owners.size !== playerIds.size) throw new Error('Missing saved crew hat');
}
 ...
 * ownership rules.
 */
export function incidents(v: Pick<PhotoFrame, 'players' | 'animals' | 'tin' | 'props' | 'spills' | 'hats' | 'tick' | 'route'>, world: ReserveBlueprint) {
    for (const property of v.props) {
        integer(property.spillUntilTick, 0, v.tick + 120);
        if (property.kind !== 'case' && property.spillUntilTick !== 0)
            throw new Error('Only the case has a spill guard');
    }
    ...
    const supported = (point: Vec3) => world.walls.every(
        (b) => !(b.max[1] > point[1] + 0.16
            && b.min[1] < point[1] + 1.7
            && point[0] > b.min[0] - 0.3
            && point[0] < b.max[0] + 0.3
            && point[2] > b.min[2] - 0.3
    ...
    for (const spill of v.spills) {
        ...
        if (!supported(spill.position))
            throw new Error('Saved spill lacks ground support');
    }
    if (v.hats.filter((h) => h.carrier.startsWith('animal:')).length > 1)
        throw new Error('Multiple raccoon hats');
    for (const hat of v.hats) {
        ...
        if (hat.carrier.startsWith('animal:')) {
            ...
            if (
                ...
                || v.players.every((p) => !(p.id === hat.owner && p.connected))
            )
                throw new Error('Contradictory stolen hat');
        } else if (hat.untilTick !== 0)
            throw new Error('Only a stolen hat has a drop deadline');
        if (hat.carrier === 'ground' && !supported(hat.position))
            throw new Error('Saved hat lacks ground support');
    }
    for (const a of v.animals)
        if (a.behavior === 'hat-reach') {
            number_(a.remaining, 0.000001, 1.2);
            if (
                a.species !== 'raccoon'
                || distance(a.pose.position, a.target) > 1.2 + 1e-6
                || v.hats.some((h) => h.carrier.startsWith('animal:'))
                || v.players.every(
                    (p) => !(p.connected
                ...
                throw new Error('Contradictory saved hat reach');
        }
}

export { object as obj, number_ as num, properties as props };
