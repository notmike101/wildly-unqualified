...
function routineStages(run: RunState, a: Animal): RoutineStage[] {
     ...
     */
    const anchor = (kind: string) => anchors.find((p) => p.kind === kind && !p.id.endsWith('-start'))!.point;
    ...
    if (a.species === 'fox' || a.species === 'badger') {
        ...
        stages = passage.map((p, index_) => stage(
            p.point,
            a.species === 'fox' ? 'stalk' : 'sniff',
            index_ === 0 ? (a.species === 'fox' ? 'pounce' : 'dig') : 'sniff',
            index_ === 0 ? 3 : 2,
        ),
        );
        ...
        const pool = run.world.waters.find((w) => water.every((v, index_) => v >= w.min[index_] && v <= w.max[index_]),
        )!;
        ...
        stages = [
            ...
            { ...stage(outside, 'swim', 'freeze', 0.2), path: path.toReversed() },
        ];
        ...
        const transform = (p: Pose): Pose => ({
            position: rotate(p.position, xyz([0, arch.yaw, 0])).map(
                (v, index_) => v + arch.position[index_],
            ) as Vec3,
        ...
        if (a.species === 'owl') {
            ...
            const approach = transform(pose([-2.5, 0, 1]));
            const climb = SQUIRREL_CLIMB.map((element) => transform(element));
            ...
            stages = [
                ...
                },
                {
                    ...
                    path: [stance, ...climb.slice(0, -1).toReversed(), approach],
                },
            ];
        }
        ...
        const snag
            = run.world.placements.find((p) => p.id === resident.home + '-snag')
                ?? run.world.placements.find(
                    (p) => p.model === 'SnagTall' && distance(p.position, resident.spawn) < 8,
                )!;
        const local: Vec3 = [
            0,
            2.5 + index * 0.45,
...
export function wildlifeStep(
    ...
    if (a.species === 'rabbit' || a.species === 'mallard') {
        ...
        if (
            ...
            && run.world.waters.every(
                (w) => !(a.pose.position[0] > w.min[0]
    ...
    if (s.path || ['fly', 'swim'].includes(s.move)) {
        ...
        const proposed = a.pose.position.map(
            (v, index_) => v + (next.position[index_] - v) * fraction,
        ) as Vec3;
        ...
        if (s.move !== 'swim') {
            const same
                = a.pose.rotation.reduce((sum, v, index_) => sum + v * next.rotation[index_], 0)
                    >= 0
                    ? 1
                    : -1;
            const q = a.pose.rotation.map(
                (v, index_) => v + (same * next.rotation[index_] - v) * fraction,
            );
        ...
        if (s.move === 'climb' || s.move === 'descend') {
            ...
            const progress
                = (s.move === 'climb' ? cursor - 2 + phase : 86 - cursor - phase) / 84;
        ...
        isArrived = cursor >= path.length;
    } else {
        isArrived = walk(run, a, s.point, s.speed, dt, extra);
        if (!isArrived && s.move === 'bound')
