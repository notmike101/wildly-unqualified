 ...
 */
export const flatDistance = (a: Vec3, b: Vec3) => Math.hypot(a[0] - b[0], a[2] - b[2]);
...
export const ground = (
    ...
    // eslint-disable-next-line unicorn/no-null -- The navigation API returns null when no supported position exists.
...
export function clear(
    ...
    const walls = [...current.walls, ...extra].filter(
        (b) => b.max[0] + radius >= Math.min(from[0], to[0])
    ...
    for (let index = 0; index <= count; index++) {
        const point = ground(
            from.map((n, axis) => n + ((to[axis] - n) * index) / count) as Vec3,
            run,
            surfaces,
        );
...
export function routeTo(
    ...
    while (visited.size < nodes.length) {
        const at = nodes
            ...
            .toSorted((a, b) => costs.get(a.id)! - costs.get(b.id)!)[0];
        ...
        const neighbors = links.get(at.id) ?? [];
        ...
        for (const id of neighbors) {
            ...
            if (
                !visited.has(id)
                && (extra.length === 0 || clear(at.position, next.position, run, extra))
            ) {
...
export function walk(
     ...
     */
    const occupied = (p: Vec3) => run.animals.some(
        (other) => other.id !== a.id
...
export function localRecoveryPoint(run: RunState, origin: Vec3): Vec3 | null {
    ...
    for (const radius of [0, 0.75, 1.5, 2.25, 3])
        for (let index = 0; index < (radius ? 8 : 1); index++) {
            const point = ground(
                [
                    origin[0] + Math.cos((index * Math.PI) / 4) * radius,
                    origin[1],
                    origin[2] + Math.sin((index * Math.PI) / 4) * radius,
                ],
                run,
            );
