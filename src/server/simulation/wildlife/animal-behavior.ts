 ...
 */
export function disturbed(run: RunState, a: Animal) {
    const point: Vec3 = [
            a.pose.position[0],
            a.pose.position[1] + 1,
            a.pose.position[2],
    ...
    return (
        run.events.some(
            (entry) => ['noise', 'impact'].includes(entry.kind)
                && entry.tick >= run.tick - 30
                && flatDistance(entry.point, point) < RULES.noiseRadius,
        )
        || run.players.some(
            (p) => p.connected
 ...
 * @param ignoredEquipment - Prop identifier excluded from lure occlusion
...
export function approachPoint(
    ...
    if (
        ...
        || propertyRayBlocked(
            eyes,
            lure,
            run.props.filter((p) => p.id !== ignoredEquipment),
            PROP_DEFINITIONS,
        )
    )
        // eslint-disable-next-line unicorn/no-null -- The navigation API returns null when no approach is reachable.
        return null;
    ...
    for (const turn of [0, Math.PI / 2, -Math.PI / 2, Math.PI]) {
        ...
        if (
            ...
            && routeTo(a.pose.position, candidate, run, extra).length > 0
        )
            return candidate;
    }
