            ...
            if (!a.remaining) {
                const threat
                    = run.players
                        ...
                        .toSorted(
                            (p, q) => distance(p.position, a.pose.position)
                ...
                ].toSorted((p, q) => distance(q.point, threat) - distance(p.point, threat));
                const choice
                    = choices.find(
                        (c) => c.id !== m.goal
                            && routeTo(a.pose.position, c.point, run, extra).length > 0,
                    )
                    ?? choices.find(
        ...
        if (a.species === 'heron') {
            ...
            if (!decoy) {
                ...
                if (m.goal.startsWith('deer-trail:')) {
                    ...
                    const index = Number(m.goal.split(':', 2)[1]),
