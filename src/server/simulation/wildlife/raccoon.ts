...
import { RULES, TIN_HALF } from '../../../shared/world/level.ts';
import {
    distance,
    pose,
    type Animal,
...
export function raccoonStep(
    ...
    if (run.tin.holder === `animal:${a.id}`) {
        ...
        const whistle = [...run.events]
            .findLast(
                (entry) => entry.kind === 'whistle'
                    && run.tick - entry.tick < 240
                    && flatDistance(entry.point, a.pose.position) < RULES.lureRadius
                    && !m.recentGoals.includes(`whistle-${entry.tick}`)
                    && m.goal !== `whistle-${entry.tick}`,
            );

        if (whistle && a.behavior !== 'investigate') {
    ...
    const decoy = run.props.find(
        ...
        noise = [...run.events]
            .findLast(
                (entry) => ['rattle', 'whistle', 'place'].includes(entry.kind)
                    && run.tick - entry.tick < 240
                    && flatDistance(entry.point, a.pose.position) < RULES.lureRadius,
            ),
    ...
    const lure = candidates.find((c) => approachPoint(run, a, c.point, extra, 0.85, c.id),
    );
     ...
     */
    const hatOwner = (id: string) => run.players.find(
        (p) => p.id === id
            && p.connected
    ...
    if (
        !lure
        && a.behavior === 'wander'
        && run.hats.every((h) => !h.carrier.startsWith('animal:'))
    ) {
    ...
    if (
        ...
        if (
            ...
            const target = routeTo(a.pose.position, stash, run, extra).length > 0
                ? stash
                : home;
    ...
    if (a.remaining) return;
    if (
        anchorsFor(run, a).every((anchor) => anchor.id !== m.goal)
        ...
        choose(
            ...
            anchorsFor(run, a)
                .filter(
                    (anchor) => ['ground', 'wash', 'rest', 'retreat'].includes(anchor.kind)
                        && (!sharesHerons
                            || !feed
