...
function tinBlocks(frame: PhotoFrame, from: Vec3, to: Vec3) {
     ...
     */
    const local = (v: Vec3) => rotate(v.map((n, index) => n - frame.tin.pose.position[index]) as Vec3, conjugate);
...
export function evaluatePhoto(
    ...
    if (frame.worldId !== world.id) throw new Error('Photo belongs to another world');
    ...
    const pointVisible = (point: Vec3, boxes = occluders) => {
        const d = point.map((v, index) => v - c.position[index]) as Vec3,
            depth = dot(d, f);
    ...
    const qualifies = (a: Animal) => {
        const points = subjectPoints(a, frame.tick).map((local) => {
            ...
            return v.map((n, index) => n + a.pose.position[index]) as Vec3;
        });
        const projected = points.map((point) => {
            const d = point.map((n, index) => n - c.position[index]) as Vec3,
                depth = dot(d, f);
        ...
        const feedback = {
            ...
            // eslint-disable-next-line unicorn/no-null -- Framing feedback uses null until a rejection reason is assigned.
        ...
        const heights = [0, SUBJECT_HEIGHT[a.species]].map((y) => {
            const point = rotate([0, y, 0], a.pose.rotation).map(
                (n, index) => n + a.pose.position[index] - c.position[index],
            ) as Vec3;
    ...
    const visible = frame.animals.filter((element) => qualifies(element));
    ...
    if (
        ...
        throw new Error('Photo resident identity mismatch');
     ...
     */
    const inspection = (a: Animal) => a.species === 'raccoon'
        && a.behavior === 'inspect'
        && frame.tin.open
    ...
    const credits = world.commissions
        .filter((commission) => {
            ...
            if (
                ...
                if (a.species === 'raccoon') return inspection(a);
                if (a.species === 'heron') return a.behavior === 'display';
                if (a.species === 'rabbit' || a.species === 'mallard') return a.behavior === 'feed';

                return a.behavior === 'investigate'
                ...
                if (!raccoon && !heron)
                    return (
                        [
                            ['deer', 'rabbit'],
                            ['beaver', 'mallard'],
                        ].some((pair) => pair.every((s) => animals.some((a) => a.species === s)),
                        )
                        && animals.every(
                            (a) => nearby(a.pose.position, anchor.point, 3)
            ...
            if (commission.kind === 'composition') {
                ...
                const min = [0, 1, 2].map((index) => Math.min(...boxes.map((b) => b.min[index])),
                ) as Vec3;
                const max = [0, 1, 2].map((index) => Math.max(...boxes.map((b) => b.max[index])),
                ) as Vec3;
    ...
    const subject = framed.toSorted((a, b) => a.center - b.center)[0];
    const hint
        = subject
