 ...
 */
export const sameSecret = (a: string, b: string) => timingSafeEqual(
...
export function body(request: IncomingMessage, max: number): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.on('error', () => reject(new HTTPError(400, 'Interrupted request')),
        );
        request.on('aborted', () => reject(new HTTPError(400, 'Interrupted request')),
        );
    });
}
...
export async function fields(request: IncomingMessage, names: string[]) {
    ...
    try {
        const bytes = await body(request, 2048);
...
export function safePath(request: IncomingMessage) {
    ...
    if (
        !path.startsWith('/')
        // eslint-disable-next-line no-control-regex -- Reject decoded control bytes at the HTTP path trust boundary.
        || /[\\\u{0}-\u{1F}]/u.test(path)
