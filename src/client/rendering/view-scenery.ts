...
import { distance, type Vec3 } from '../../shared/shared.ts';
import { findPart, instanceModel } from './forest-view.ts';
import { bankStance, rotate } from '../../shared/wildlife/wildlife.ts';
...
export function addFieldScenery(
    ...
    for (const p of world.placements) {
        if (p.model === 'TrailBoard') sign('WILLOWMERE', p.position, p.yaw);
    }
    const anchors = world.pockets.flatMap((p) => p.anchors);

    for (const anchor of anchors) {
        if (!['feed', 'wash'].includes(anchor.kind)) continue;
        const marker = mesh(new THREE.RingGeometry(1.2, 1.27, 24), 0xCA_C1_98, [
            anchor.point[0],
            anchor.point[1] + 0.04,
            anchor.point[2],
        ]);
    ...
    for (const point of world.navNodes) {
        if (!point.id.includes('-camera-')) continue;
        const marker = mesh(new THREE.RingGeometry(0.3, 0.35, 16), 0xDE_C3_78, [
            point.position[0],
        ...
        for (let index = 1; index < path.length; index++) {
            ...
            for (let along = 0.6; along < length; along += 1.3)
                tracks.push({
                    ...
                    model: 'RaccoonTracks',
                    position: a.map(
                        (v, axis) => v + ((b[axis] - v) * along) / length + (axis === 1 ? 0.035 : 0),
                    ) as Vec3,
    ...
    if (trackModel && tracks.length > 0) scene.add(instanceModel(trackModel, tracks));
    for (const resident of world.residents) {
        if (resident.species !== 'beaver') continue;
        const stance = bankStance(world, resident.id),
            offset = rotate([0, 0, -0.75], stance.rotation);
        const pile = field(
            'BranchPile',
            stance.position.map((v, index) => v + offset[index]) as Vec3,
            0.6,
        );
