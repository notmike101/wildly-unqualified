...
export async function capturePhoto(
    ...
    if (frame.worldId !== world.id) throw new Error('Capture world mismatch');
    ...
    try {
        ...
        canvas.width = 640;
        canvas.height = 360;
        const rgba = new Uint8ClampedArray(new Uint8Array(pixels.buffer, pixels.byteOffset, pixels.byteLength));
        ...
        for (const quality of [0.85, 0.65, 0.4, 0.2]) {
            ...
            if (blob && blob.size <= 65_536) return blob;
        }
        throw new Error('Photo could not fit the album. Please try again.');
    } finally {
        target.dispose();
    }
}
