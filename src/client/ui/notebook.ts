 ...
 */
import type { Snapshot, Vec3 } from '../../shared/shared.ts';
...
export function renderNotebook(
    ...
    // eslint-disable-next-line unicorn/no-top-level-assignment-in-function -- Cache the rendered album to preserve existing DOM controls.
    albumSignature = signature;
    $('album').replaceChildren();
    if (latest.album.length === 0) {
    ...
    for (const photo of latest.album.toReversed()) {
        ...
        if (photo.thumbnail === 'ready') {
            ...
            image.alt = photo.credits.length > 0
        ...
        caption.textContent = `${latest.players.find((p) => p.id === photo.photographer)?.name ?? 'Researcher'} · ${photo.credits.map((c) => commissionTitle(c)).join(' / ') || 'Field moment'}${photo.assists.length > 0 ? ' · helped by ' + photo.assists.map((id) => latest!.players.find((p) => p.id === id)?.name ?? 'a friend').join(', ') : ''}`;
        f.append(caption);
        if (photo.incident) {
        ...
        if (photo.thumbnail === 'ready') {
             ...
             */
            b.addEventListener('click', () => onFavorite(photo.id));
            f.append(b);
            if (photo.favorites.length > 0) {
                ...
                crew.textContent
                    = 'Selected by '
                        + photo.favorites
                            .map(
                                (id) => latest!.players.find((p) => p.id === id)?.name ?? 'Researcher',
                            )
                            .join(', ');
                f.append(crew);
            }
...
export function drawMap(
    ...
    for (const trail of world.trails) {
        c.beginPath();
        for (const [index, p] of trail.points.entries()) {
            const [x, y] = point(p);
    ...
    const labels: [string, Vec3][] = [
        ...
        ...world.pockets.map(
            (p) => [p.id.toUpperCase() + ' ' + p.habitat, p.position] as [string, Vec3],
        ),
    ];
    ...
    for (const [name, p] of labels) {
        ...
        const positions = [-8, 16, -22, 30].flatMap((dy) => [x - 18, x + 8, x - width - 8].map((dx) => ({
    ...
    for (const node of world.navNodes) {
        if (!node.id.includes('-camera-')) continue;
